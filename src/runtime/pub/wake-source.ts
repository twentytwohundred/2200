...
function composeTaskBody(args: {
  ...
  if (isReplyToMine) {
    lines.push(
      ...
      ``,
      '```tool',
      `{ "tool": "pub.react", "args": { "pub_name": "${args.pubName}", "message_id": "${args.message_id}", "emoji": "✓" }, "predicted_outcome": "reaction landed on the reply", "reason": "ack that I saw the answer" }`,
      '```',
      ``,
    )
  } else {
    lines.push(
      ...
      ``,
      '```tool',
      `{ "tool": "pub.send", "args": { "pub_name": "${args.pubName}", "content": "<your reply>", "in_reply_to": "${args.message_id}" }, "predicted_outcome": "message delivered", "reason": "responding to the sender" }`,
      '```',
      ``,
      `Example reaction:`,
      ``,
      '```tool',
      `{ "tool": "pub.react", "args": { "pub_name": "${args.pubName}", "message_id": "${args.message_id}", "emoji": "✓" }, "predicted_outcome": "reaction landed", "reason": "acknowledging without burning a turn" }`,
      '```',
      ``,
    )
  }
