...
const ROUTER_SYSTEM_PROMPT = [
  ...
  `A message just arrived. Decide which Agents (if any) should respond.`,
  ``,
  `Rules:`,
  `- Wake an Agent only if the message is addressed to them (by name) or`,
  `  clearly falls inside their stated role.`,
  `- Generic chatter, status updates, or social pleasantries directed at`,
  `  no one in particular wake nobody.`,
  `- Wake multiple Agents only when the message explicitly requires more`,
  `  than one expertise area at the same time.`,
  `- Most messages should wake 0 or 1 Agents. Be conservative; over-waking`,
  `  burns tokens and feels noisy to the human in the room.`,
  ``,
  `Respond with a single JSON object on one line, no surrounding prose:`,
  `{"woken_agent_ids": ["<agent_id>", "..."], "rationale": "<one short sentence>"}`,
  ``,
  `If no Agent should respond, return {"woken_agent_ids": [], "rationale": "..."}.`,
].join('\n')
