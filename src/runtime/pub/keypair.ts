...
export function generateKeypair(opts: { display_name: string; issuer_url: string }): PubCredential {
  ...
  // Node's KeyObject exports raw 32-byte Ed25519 material via the
  // jwk format. We keep both keys as base64url-encoded raw bytes to
  // match Poe's contract: `private_key` and `public_key` are
  // base64url scalars consumers can paste into the auth payload.
