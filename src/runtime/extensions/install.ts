 ...
 *
 * Schedule + tool registration land in the next sub-phase. A hook
 * that wants to do schedule-y or tool-y things at install time must
 * write its desire into `state.json` for now; the registration step
 * after this lands will reconcile manifest declarations with the
 * persisted state.
 */
...
export async function installExtension(args: InstallArgs): Promise<InstallResult> {
  ...
  if (decision === null) {
    return { manifest, granted: [], hookResult: null, aborted: true }
  }
...
export async function uninstallExtension(args: UninstallArgs): Promise<UninstallResult> {
  ...
  if (await dirExists(destRoot)) {
    ...
    return { removed: false, hookResult: null, aborted: false, hookFailed: false }
  }

  if (!args.skipApprove) {
    const ok = await args.approve()
    if (!ok) {
      return { removed: false, hookResult: null, aborted: true, hookFailed: false }
    }
  }
  ...
  return { removed: true, hookResult, aborted: false, hookFailed }
}
