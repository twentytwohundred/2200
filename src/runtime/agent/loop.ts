...
export class AgentLoop {
  ...
  private async buildSystemPrompt(): Promise<string> {
    ...
    const runtimeBlock: string[] = [
      ...
      '',
      '8. **Ask the operator for a credential rather than improvising one.** When a task needs a credential you do not have in vault, call `credential_request` from a 1:1 chat with the operator. The pasted value goes directly into your vault under the supplied `credential_name`; you never see it. Do NOT: paste a placeholder string, ask the operator to send the credential in a chat message (operator-typed values in chat content end up in your transcript and your model provider context, which is exactly the leak surface this tool exists to avoid), or hand-roll an OAuth flow that requires the operator to copy-paste a token elsewhere. The tool blocks while the operator decides, returns one of `fulfilled` / `declined` / `expired`, and the audit substrate can verify "I asked for X" against the request ledger and "I have X" against `vault.has`. If the tool returns `declined` or `expired`, surface that to the operator and stop ... do not retry without their consent. `credential_request` only works from 1:1 chat tasks (`task.source.kind === \'chat\'`); from a pub message or scheduled task the tool returns `surface_invalid` and you must `chat_send` to bring the conversation to chat first.',
      '',
    ]
    const lines: string[] = [
      id.body,
