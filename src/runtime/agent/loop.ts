    ...
    while (this.iteration < this.maxIterations) {
      ...
      if (parsed.calls.length === 0 && parsed.errors.length === 0) {
        if (response.text.trim().length > 0) {
          // Wake-task enforcement: when the runtime started this task
  ...
  private async buildSystemPrompt(): Promise<string> {
    ...
    const runtimeBlock: string[] = [
      ...
      ...(fleetMd
        ...
        : []),
      '## Tool triggers (quick reference)',
      '',
      'You have a tool for almost everything an operator might ask you to do. The full set is listed below ... this section is a fast lookup of the special-flow tools whose USE PATTERN is non-obvious. When you see one of these triggers in a user message, the correct response is to call the named tool, NOT to ask the user to send the value as text or do the thing yourself.',
      '',
      '  - User says "I have an API key / token / password / secret / credential" or "I will give you a key / will paste a token" → call `credential_request` immediately. NEVER ask them to send the value as a chat message ... it would land in your transcript and the LLM provider context. The `credential_request` card pops a paste field that bypasses your loop entirely.',
      '  - User asks you to relay an answer between people / Agents ("ask Hobby and tell me what he says", "check with Simon and get back to me") → after sending the question, call `task_await_response` so the substrate resumes THIS task with the response appended, instead of dying after the first send.',
      "  - User asks you to delegate work to another Agent → call `task_create_for_agent`. Do NOT try to do the peer Agent's job yourself.",
      "  - User asks you to remember / take a note / write down → call `brain_write` (or `brain_write_shared` for fleet-wide). Memory is durable; don't rely on transcript scrollback.",
      '  - User asks for info that might be in your brain → call `brain_search` BEFORE asking them to repeat.',
      '  - User asks you to call an external HTTP API → `http_request`. NEVER ask them to paste API responses as text.',
      '  - User asks you to schedule something / run something on a cadence → `schedule_add`.',
      ...
      '',
      'You have a persistent 1:1 chat with the user (the human operator) at `<home>/agents/<your-name>/chat.jsonl`, surfaced in their web UI at `/agent/<your-name>/chat`. The user posts there to talk to you privately ... messages to you alone, not the room. When the user starts a task that originated in chat, the loop already routes your final answer back into that chat. But if you need to push something INTO that chat without the user prompting first ... a follow-up after pub work, a status update, a heads-up about something you noticed ... use the `chat_send` tool. It appends an assistant-role message to your chat log; the user sees it the next time they open or refresh the chat screen. Only the user sees it; other Agents do not.',
      '',
      ...
      '',
      'The canonical shape:',
      '  1. Receive a request on surface A ("the operator in Discord channel X: ask another Agent what they need from me").',
      '  2. Send the question on surface B (`pub_send` to Studio, @-mentioning that Agent).',
      '  3. Acknowledge to the user on surface A so they know you heard them and are working on it (e.g. `discord_send`: "I\'ve asked; I\'ll relay the answer when it lands.").',
